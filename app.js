$(document).ready(function () {
  ...
  $("#btnAddprocess").on("click", function () {
    ...
    $("#tblProcessList > tbody:last-child").append(
      `<tr>
                  ...
                  <td id="tdprioTime">${priority.val()}</td>
              </tr>`
    );
  ...
  $("#btnCalculate").on("click", function () {
    ...
    var selectedAlgo = $("#algorithmSelector")
  ...
  function priorityScheduling() {
    ...
    while (processList.length > 0 || queue.length > 0) {
      ...
      selectProcessForPriorityScheduling();
    ...
    function selectProcessForPriorityScheduling() {
      if (queue.length != 0) {
        queue.sort(function (a, b) {
          if (a.priority < b.priority) {
            ...
            // In case of tie, use arrival time as a tiebreaker
    ...
    TableData.forEach((pInTable) => {
      completedList.forEach((pInCompleted) => {
        if (pInTable.processID == pInCompleted.processID) {
          ...
          pInCompleted.turnAroundTime =
            pInCompleted.completedTime - pInCompleted.arrivalTime;
          pInCompleted.waitingTime =
            pInCompleted.turnAroundTime - pInCompleted.burstTime;
        }
      });
    });
